...
$@._STRPTIME_DATETIME:FORMAT ::= $_LOCALIZED_MONTH.FORMAT %d %Y %I:%M%p
$@._STRPTIME:DATA_STRING ::= $@._STRPTIME:ARG
$_LOCALIZED_MONTH.FORMAT ::= $@.<LAMBDA>:X
$@._STRPTIME:ARG ::= $@.<LISTCOMP>:_LOCALIZED_MONTH.__GETITEM___@ 1 $@._STRPTIME:FOUND_DICT.Y  1:$@._STRPTIME:FOUND_DICT.M$@._STRPTIME:FOUND_DICT.P
$@.<LAMBDA>:X ::= %b
$@.<LISTCOMP>:_LOCALIZED_MONTH.__GETITEM___@ ::= $@._STRPTIME:FOUND_DICT.B
...
$@._STRPTIME:DATA_STRING ::= $@._STRPTIME:ARG
$@._STRPTIME:FORMAT ::= $@._STRPTIME:ARG
	| %d %b %y
...
$@._STRPTIME:FOUND_DICT.D ::= 13
	| 14
	| 28
$TIMERE.COMPILE:FORMAT ::= $TIMERE.PATTERN:FORMAT
$TIMERE.PATTERN:FORMAT ::= $@._STRPTIME:FORMAT
'''[1:-1]
    result = []
    for line in date_lines.split('\n'):
        if line.strip() == '': continue
